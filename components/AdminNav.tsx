  ...
  const NavContent = () => (
    <>
      ...
      <div style={{ padding: "1rem 0.75rem", borderTop: "1px solid rgba(200,169,126,0.1)" }}>
        <Link href={`/${username}`} target="_blank" onClick={() => setOpen(false)} style={{ display: "block", color: "#8fa3b1", textDecoration: "none", fontFamily: "Inter, sans-serif", fontSize: "0.8rem", padding: "0.5rem 0.875rem", marginBottom: "0.25rem" }}>
          ↗ View Blog
        </Link>
  ...
  return (
    <>
      ...
      <div className="admin-topbar-mobile" style={{
        ...
        <button onClick={() => setOpen(true)} style={{ background: "none", border: "none", color: "#c8a97e", fontSize: "1.4rem", cursor: "pointer", padding: "0.25rem 0.5rem", lineHeight: 1 }}>☰</button>
        <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "1rem", color: "#c8a97e" }}>/{username}</span>
        <Link href={`${base}/posts/new`} style={{ background: "rgba(200,169,126,0.15)", color: "#c8a97e", textDecoration: "none", fontFamily: "Inter, sans-serif", fontSize: "0.75rem", padding: "0.35rem 0.75rem", borderRadius: 4, border: "1px solid rgba(200,169,126,0.3)" }}>✎ Write</Link>
      </div>
      ...
      <aside className="admin-sidebar-mobile" style={{
        ...
        <button onClick={() => setOpen(false)} style={{ position: "absolute", top: "0.75rem", right: "0.75rem", background: "none", border: "none", color: "#8fa3b1", fontSize: "1.5rem", cursor: "pointer", lineHeight: 1, padding: "0.25rem" }}>×</button>
        <NavContent />
      </aside>
